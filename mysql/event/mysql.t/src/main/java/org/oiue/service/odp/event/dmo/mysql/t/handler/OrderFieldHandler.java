...
public class OrderFieldHandler extends AbstractHandler {
	 ...
	 */
	private void getField(boolean isEnd, char c) {
		if (builder.length() == 0) {
			if (isEnd) {
				return;
			}
			throw new OIUEException(StatusResult._blocking_errors, "There should be a alias after '%' or format error");
		} else {
			...
			if (value == null) {
				...
				throw new OIUEException(StatusResult._blocking_errors, "The value of order by field is error, the field: " + fieldTemp + ", the value: " + value);
			}
		}
	}
