...
public class SelectEventPostgresqlService extends DMO implements Event {
	
	private static final long serialVersionUID = -4232372066134799100L;
	private Logger logger;
	private LogService logService;
	
	public SelectEventPostgresqlService(LogService logService) {
	...
	public Event clone() throws CloneNotSupportedException {
		return new SelectEventPostgresqlService(logService);
	}
	...
	private Map executeQuery(Map map, CallBack callBack) {
		...
		if (callBack == null) {
			try {
				if (this.getRs()!=null&&this.getRs().next()) {
					try {
